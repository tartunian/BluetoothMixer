...
#define SPEAKER_NAME		    "JLab JBuds Air"        //"DESKTOP-M3M5MD1"
...
void app_main()
{
    ...
    btStart();
    ESP_LOGE(BT_AV_TAG, "btStarted() said: %d \n", btStarted());
    ...
    ESP_LOGE(BT_AV_TAG, "esp_bluedroid_get_status() said: %d \n", esp_bluedroid_get_status());
    */
    ...
    if (esp_bt_controller_init(&bt_cfg) != ESP_OK) {
        ...
        ESP_LOGE(BT_AV_TAG, "ESP_BT_MODE_CLASSIC_BT %d\n", ESP_BT_MODE_CLASSIC_BT);
        ESP_LOGE(BT_AV_TAG, "esp_bt_controller_enable(ESP_BT_MODE_CLASSIC_BT): %d\n", esp_bt_controller_enable(ESP_BT_MODE_CLASSIC_BT));
    ...
    */
    ...
    ESP_LOGE(BT_AV_TAG, "esp_bluedroid_get_status() after esp_bluedroid_init() said: %d \n", esp_bluedroid_get_status());

    if (esp_bluedroid_enable() != ESP_OK) {
        ESP_LOGE(BT_AV_TAG, "%s enable bluedroid failed\n", __func__);
        return;
    }
    ESP_LOGE(BT_AV_TAG, "esp_bluedroid_get_status() after esp_bluedroid_enable() said: %d \n", esp_bluedroid_get_status());
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    /* search for device with name = SPEAKER_NAME in its extended inqury response */
    if (eir) {
        get_name_from_eir(eir, s_peer_bdname, NULL);
        if (strcmp((char *)s_peer_bdname, SPEAKER_NAME) != 0) {
            return;
        }
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    ...
    // return len;
    size_t bytes_read;
    i2s_read(i2s_port, data, len, &bytes_read, portMAX_DELAY);
    return bytes_read;
}
...
static void bt_app_av_sm_hdlr(uint16_t event, void *param)
{
    ESP_LOGE(BT_AV_TAG, "%s state %d, evt 0x%x", __func__, s_a2d_state, event);
    switch (s_a2d_state) {
...
static void bt_app_av_state_unconnected(uint16_t event, void *param)
{
    ...
    case BT_APP_HEART_BEAT_EVT: {
        ...
        ESP_LOGE(BT_AV_TAG, "unconnected: a2dp connecting to peer: %02x:%02x:%02x:%02x:%02x:%02x",
                 p[0], p[1], p[2], p[3], p[4], p[5]);
        esp_err_t result = esp_a2d_source_connect(s_peer_bda);
        switch(result) {
            case ESP_OK: {
                ESP_LOGE(BT_AV_TAG, "Connection request OK");
                break;
            }
            case ESP_ERR_INVALID_STATE: {
                ESP_LOGE(BT_AV_TAG, "BT stack not enabled.");
                break;
            }
            case ESP_FAIL: {
                ESP_LOGE(BT_AV_TAG, "Unknown failure");
                break;
            }
        }
        s_a2d_state = APP_AV_STATE_CONNECTING;
